...
var jsxRuntime = require('/workspace/node_modules/@asyncapi/generator/node_modules/react/cjs/react-jsx-runtime.production.min.js');
...
function index ({
  ...
  const messages = [...new Map(channels.flatMap(channel => channel.messages().all()).map(message => {
    var _message$name;
    return [(_message$name = message.name()) !== null && _message$name !== void 0 ? _message$name : message.id(), message];
  })).values()];
  const channel = channels[0];
  const address = channel.address();
  const producerMethods = messages.map(message => {
    var _message$name2;
    const name = javaName((_message$name2 = message.name()) !== null && _message$name2 !== void 0 ? _message$name2 : message.id());
    return `        boolean publish${name}(${name}Payload payload, EventHeaders headers);`;
  }).join('\n');
  const consumerMethods = messages.map(message => {
    var _message$name3;
    const name = javaName((_message$name3 = message.name()) !== null && _message$name3 !== void 0 ? _message$name3 : message.id());
    return `        void on${name}(${name}Payload payload, EventHeaders headers);`;
  }).join('\n');
  const payloads = messages.map(message => {
    var _message$name4;
    const name = javaName((_message$name4 = message.name()) !== null && _message$name4 !== void 0 ? _message$name4 : message.id());
    return `    public record ${name}Payload(String json) {}`;
  }).join('\n\n');
  return /*#__PURE__*/jsxRuntime.jsx(generatorReactSdk.File, {
    name: "GeneratedChannelContract.java",
    children: /*#__PURE__*/jsxRuntime.jsx(generatorReactSdk.Text, {
      children: `package ${JAVA_PACKAGE};
